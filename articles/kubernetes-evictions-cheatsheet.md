...
### Prometheus Alerts for Evictions

```yaml
groups:
- name: kubernetes-evictions
  rules:

  # Alert on pods evicted due to node pressure
  - alert: PodEvictedNodePressure
    expr: |
    ...
    labels:
      severity: warning
    annotations:
      summary: "Pod {{ $labels.pod }} evicted from {{ $labels.namespace }}"
      description: "Pod was evicted due to node resource pressure. Check node conditions."

  # Alert on accumulating failed pods (kubelet admission)
  - alert: FailedPodsAccumulating
    expr: |
      count(kube_pod_status_phase{phase="Failed"}) by (namespace) > 50
    for: 5m
    labels:
      severity: warning
    annotations:
      summary: "{{ $value }} failed pods in namespace {{ $labels.namespace }}"
      description: "Failed pods are accumulating. May indicate kubelet admission failures or storage evictions."

  # Alert on node memory pressure
  - alert: NodeMemoryPressure
    ...
    annotations:
      ...
      description: "Kubelet will begin evicting pods. Investigate memory usage."

  # Alert on node disk pressure
  - alert: NodeDiskPressure
    ...
    annotations:
      ...
      description: "Kubelet will begin evicting pods. Investigate disk usage."

  # Alert on node not ready (precursor to taint eviction)
  - alert: NodeNotReady
    expr: |
    ...
    annotations:
      ...
      description: "NoExecute taint will be applied. Pods will be evicted in ~5 minutes unless node recovers."

  # Alert on PDB violations during preemption
  - alert: PDBViolation
    expr: |
      kube_poddisruptionbudget_status_pod_disruptions_allowed == 0
      and kube_poddisruptionbudget_status_current_healthy < kube_poddisruptionbudget_status_desired_healthy
    for: 5m
    labels:
      severity: warning
    annotations:
      summary: "PDB {{ $labels.poddisruptionbudget }} is being violated"
      description: "Healthy pods are below desired. Availability target is compromised."

  # Alert on frequent preemptions
  - alert: FrequentPodPreemptions
    expr: |
    ...
    labels:
      severity: warning
    annotations:
      summary: "Frequent pod preemptions detected"
      description: "{{ $value }} pods preempted in the last hour. Check cluster capacity and priority classes."

  # Alert on node heartbeat staleness (early warning for taint eviction)
  - alert: NodeHeartbeatStale
    expr: |
      time() - kube_node_status_condition{condition="Ready"} > 30
    for: 30s
    labels:
      severity: warning
    annotations:
      summary: "Node {{ $labels.node }} heartbeat is stale"
      description: "Node may become unreachable. Taint-based eviction could trigger in ~5.5 minutes."
```
