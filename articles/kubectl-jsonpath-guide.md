...
kubectl get pods -o jsonpath='{.items[?(@.metadata.labels.app=="myapp")].metadata.name}'
...
kubectl get deployments -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.spec.strategy.type}{"\n"}{end}'
...
kubectl get svc -o jsonpath='{range .items[?(@.spec.type=="LoadBalancer")]}{.metadata.name}{"\t"}{.status.loadBalancer.ingress[*].ip}{"\n"}{end}'
...
kubectl get secrets -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.type}{"\n"}{end}'
...
kubectl get pods -o jsonpath='{.items[*].spec.volumes[*].name}'

# Volume mount paths per container
kubectl get pods -o jsonpath='{.items[*].spec.containers[*].volumeMounts[*].mountPath}'

# Environment variable names from pods
kubectl get pods -o jsonpath='{.items[*].spec.containers[*].env[*].name}'
...
kubectl get endpoints -o jsonpath='{range .items[*]}{.metadata.name}{": "}{range .subsets[*]}{range .addresses[*]}{.ip}{" "}{end}{end}{"\n"}{end}'
```
...
# Specific indices
kubectl get pods -o jsonpath='{.items[0,2,4].metadata.name}'
```
...
kubectl get pods -o jsonpath='{range .items[*]}{$name := .metadata.name}{$phase := .status.phase}{$name}{" is "}{$phase}{"\n"}{end}'
...
kubectl get deployments -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.status.replicas}{"/"}{.status.readyReplicas}{"/"}{.status.updatedReplicas}{"\n"}{end}'
...
kubectl get ingress -o jsonpath='{range .items[*]}{range .spec.rules[*]}{range .http.paths[*]}{.backend.service.name}{"\n"}{end}{end}{end}'
```
